...
class TrainingConfig:
    ...
    # Dataset choice
    dataset: str = "harmful_to_safe"  # Options: "harmful_to_safe", "vangogh_style_to_safe", "kelly_mckernan_style_to_safe_prompts"
    
    # LoRA parameters (default: full fine-tune)
    use_lora: bool = False
    lora_rank: int = 8
    lora_alpha: int = 16
    ...
    # Learning rates
    lr_encoder1: float = 1e-4
    lr_encoder2: float = 1e-4
    lr_encoder3: float = 1e-3
    lr_disc: float = 1e-3
    ...
    # Preservation loss options
    # Choices: "pooled_l2", "token_l2", "token_cosine", "token_kl"
    preservation_type: str = "token_cosine"
    kl_temperature: float = 1.0

    # Single-encoder mode: 1, 2, or 3. 0 = train all (default).
    encoder_idx: int = 0


def parse_args():
    parser = argparse.ArgumentParser(description="SD3.5 Unlearning Training")
    ...
    parser.add_argument("--lr_encoder1", type=float, default=1e-4, help="Learning rate for encoder 1")
    parser.add_argument("--lr_encoder2", type=float, default=1e-4, help="Learning rate for encoder 2")
    parser.add_argument("--lr_encoder3", type=float, default=1e-3, help="Learning rate for encoder 3")
    parser.add_argument("--lr_disc", type=float, default=1e-3, help="Learning rate for discriminators")
    ...
    parser.add_argument("--dataset", type=str, default="harmful_to_safe", choices=["harmful_to_safe", "vangogh_style_to_safe", "kelly_mckernan_style_to_safe_prompts"], help="Dataset choice")
    parser.add_argument("--use_fixed_target_emb", action="store_true", help="Use fixed target embeddings for preservation loss")
    parser.add_argument("--pooled_embed", action="store_true", help="Use pooled embeddings instead of token-level")
    parser.add_argument("--encoder_idx", type=int, default=0, choices=[0, 1, 2, 3],
                        help="Train only this encoder (1=CLIP-G, 2=CLIP-L, 3=T5). 0=all (default).")
    parser.add_argument(
        "--preservation_type",
        type=str,
